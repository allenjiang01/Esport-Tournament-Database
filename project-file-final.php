...
<h2>Add Player to existing team - Insert Operation</h2>
...
<h2>Update Player Birthday - Update Operation</h2>
...
<hr />
<h2> Filter Games based on crossPlatForm Play and numParticipants - Select Query</h2>
<form method="GET" action="project-file-test.php"> <!--refresh page when submitted-->
    <input type="hidden" id="displayGamesTupleRequest" name="displayGamesTupleRequest">
    crossPlatFormPlay type in '1' for 'True' or '0' for 'False':<input type="number" name="crossPlatFormBool"><br /><br />
    number of participants:<input type"number" name ="numParticipants"><br /><br />
    <input type="submit" value="show Games" name="displayGamesTuples"></p>
</form>
...
<h2>Delete Organization - Deletion Operation with DELETE ON CASCADE (for employees)</h2>
<form method="POST" action="project-file-test.php"><!--refresh page when submitted-->
...
<hr/>
<h2>View Viewership From Each Country Based Off Amount Spent or Less per Company - Aggregation with Group By Query</h2>
...
<h2>Find a Sponsor Who Has Sponsored Every Event - Division Query</h2>
<form method="GET" action="project-file-test.php">
...
<h2>Display Games</h2>
<form method="GET" action="project-file-test.php"> <!--refresh page when submitted-->
    <input type="hidden" id="displayAllGamesTupleRequest" name="displayAllGamesTupleRequest">
    <input type="submit" value="show games" name="displayAllGamesTuples"></p>
</form>

<hr/>
<h2>Display Spectators Info</h2>
<form method="GET" action="project-file-test.php"> <!--refresh page when submitted-->
    <input type="hidden" id="displayAllSpectatorsInfoRequest" name="displayAllSpectatorsInfoRequest">
    <input type="submit" value="Show Spectator Info" name="displayAllSpectatorsInfoTuples"></p>
</form>

<hr/>
<h2>Display Spectators Tickets</h2>
<form method="GET" action="project-file-test.php"> <!--refresh page when submitted-->
    <input type="hidden" id="displayAllSpectatorsTicketsRequest" name="displayAllSpectatorsTicketsRequest">
    <input type="submit" value="Show Spectator Ticket" name="displayAllSpectatorsTicketsTuples"></p>
</form>

<hr/>
<h2>Display Attends</h2>
<form method="GET" action="project-file-test.php"> <!--refresh page when submitted-->
    <input type="hidden" id="displayAllAttendsRequest" name="displayAllAttendsRequest">
    <input type="submit" value="Show Attends" name="displayAllAttendsTuples"></p>
</form>
...
<h2>Display Tournaments</h2>
<form method="GET" action="project-file-test.php"> <!--refresh page when submitted-->
    <input type="hidden" id="displayAllTournamentsRequest" name="displayAllTournamentsRequest">
    <input type="submit" value="Show Tournaments" name="displayAllTournamentsTuples"></p>
</form>
...
function connectToDB()
{
    ...
    $db_conn = OCILogon("ora_acc1997", "a21145636", "dbhost.students.cs.ubc.ca:1522/stu");
...
function handleUpdateRequest()
{
    ...
    executePlainSQL("UPDATE Player SET dateOfBirth='" . $new_bday . "' WHERE inGameName='" . $gamer_tag . "'
                            AND playerName='" . $real_name . "'");
...
function handleResetRequest()
{
    ...
    executePlainSQL("CREATE TABLE Tournament(tournamentName char(40), ticketPrice int, grandPrize int, orgName char(40), address char(40) NOT NULL, arenaName char(40) NOT NULL, tournamentDate char(20), tournamentTime char(5), PRIMARY KEY(tournamentName), FOREIGN KEY(orgName) REFERENCES Organization(orgName) ON DELETE SET NULL, FOREIGN KEY(address, arenaName) REFERENCES Venue(address, arenaName))");
    ...
    executePlainSQL("CREATE TABLE Broadcasts(broadcasterName char(40), tournamentName char(40), gameName char(50), cost int, viewership int, PRIMARY KEY(broadcasterName, tournamentName, gameName), FOREIGN KEY(broadcasterName) REFERENCES BroadcastService(broadcasterName), FOREIGN KEY(tournamentName) REFERENCES Tournament(tournamentName), FOREIGN KEY(gameName) REFERENCES Game(gameName))");
    ...
    executePlainSQL("INSERT INTO Funds VALUES ('ESL Invitational 2021', 'Justice League', 4000)");
    executePlainSQL("INSERT INTO Funds VALUES ('Faker Cup', 'Justice League', 2000)");
    executePlainSQL("INSERT INTO Funds VALUES ('Tournament of Power', 'Justice League', 8000)");
...
{ //prints results from a select statement
    ...
    while ($row = OCI_Fetch_Array($result, OCI_BOTH)) {
        echo "<tr><td>  " . $row["TOURNAMENTNAME"] . "  </td><td>  " . $row["SPONSORNAME"] . "  </td><td>  " . $row["SPONSORFEE"] . "  </td></tr>"; //or just use "echo $row[0]"
    }
...
{ //prints results from a select statement
    ...
    while ($row = OCI_Fetch_Array($result, OCI_BOTH)) {
        echo "<tr><td>  " . $row["ORGNAME"] . "  </td><td>  " . $row["WEBSITE"] . "  </td><td>  " . $row["EMAIL"] . "  </td></tr>"; //or just use "echo $row[0]"
    }
...
{ //prints results from a select statement
    ...
    while ($row = OCI_Fetch_Array($result, OCI_BOTH)) {
        echo "<tr><td>  " . $row["EMPLOYEEID"] . "  </td><td>  " . $row["NAME"] . "  </td><td>  " . $row["EMAIL"] . "  </td><td>  " . $row["ORGNAME"] . "</td></tr>"; //or just use "echo $row[0]"
    }
...
function handleDeleteRequest()
{
    ...
    executePlainSQL("DELETE FROM Organization WHERE orgName ='" . $org_name . "'");
    echo "<br> Deleted Organization: " . $org_name . " <br>";
...
function handleAggregationWithGroupBy()
{
    ...
    $result = executePlainSQL("SELECT sum(b.viewership) sum, bs.country, count(*) num, sum(b.cost) cost FROM BroadcastService bs, Broadcasts b
                        WHERE b.cost <= '" . $monetary_amount . "' AND bs.broadcasterName = b.broadcasterName GROUP BY bs.country");
...
function printGroupByResult($result)
{
    ...
    while ($row = OCI_Fetch_Array($result, OCI_BOTH)) {
        echo "<tr><td>  " . $row["SUM"] . "  </td><td>  " . $row["COUNTRY"] . "  </td><td>  " . $row["NUM"] . "  </td><td>  " . $row["COST"] . " </td></tr>";
    }
...
function handleDivisionQuery()
{
    ...
    $results = executePlainSQL("SELECT s.sponsorName name FROM SponsorInfo s WHERE NOT EXISTS ((SELECT t.tournamentName FROM Tournament t)
                                MINUS (SELECT f.tournamentName FROM Funds f WHERE f.sponsorName = s.sponsorName))");
...
function handleDisplayTournamentRequest()
{
    ...
    $result = executePlainSQL("SELECT tournamentName, ticketPrice FROM Tournament WHERE ticketPrice <= '$max_val'");
...
function displayGames($result){
              echo "<br>Games : <br>";
              echo "<table border=\"1\">";
              echo "<tr><th> Game Name </th><th>Number of Participants</th><th> Cross platform 0 for F and 1 for T</th></tr>";

             while ($row = OCI_Fetch_Array($result, OCI_BOTH)) {
                  echo "<tr><td>  " . $row["GAMENAME"] . "  </td><td>  " . $row["NUMPARTICIPANTS"] . "  </td><td>  " . $row["CROSSPLATFORMPLAY"] . "  </td></tr>";
            }
...
function handleDisplayGamesRequest(){
           global $db_conn;
           $is_cross = $_GET['crossPlatFormBool'];
           $num_participants = $_GET['numParticipants']; 
           $result = executePlainSQL("SELECT * FROM Game WHERE numParticipants = '$num_participants' AND crossPlatformPlay ='$is_cross'");

           displayGames($result);
       }


function handleDisplayAllGamesRequest(){
     global $db_conn;
     $result = executePlainSQL("SELECT * FROM Game");
     displayGames($result);
}

function displaySpectatorsInfo($result)
{
   echo "<br>SpectatorsInfo : <br>";
              echo "<table border=\"1\">";
              echo "<tr><th>Name</th><th>Driver ID</th><th>Phone Number</th></tr>";

             while ($row = OCI_Fetch_Array($result, OCI_BOTH)) {
                  echo "<tr><td>  " . $row["NAME"] . "  </td><td>  " . $row["DRIVERID"] . "  </td><td>  " . $row["PHONENUMBER"] . "  </td></tr>";
            }
...
function handleDisplayAllSpectatorsInfoRequest(){
    global $db_conn;
    $result = executePlainSQL("SELECT * FROM spectatorInfo");
    displaySpectatorsInfo($result);
 
}


function displaySpectatorsTickets($result)
{
   echo "<br>SpectatorsTickets : <br>";
              echo "<table border=\"1\">";
              echo "<tr><th>Ticket Number</th><th>Driver ID</th></tr>";

             while ($row = OCI_Fetch_Array($result, OCI_BOTH)) {
                  echo "<tr><td>  " . $row["TICKETNUMBER"] . "  </td><td>  " . $row["DRIVERID"] . "  </td></tr>";
            }
...
function handleDisplayAllSpectatorsTicketsRequest(){
    global $db_conn;
    $result = executePlainSQL("SELECT * FROM spectatorTicket");
    displaySpectatorsTickets($result);
 
}


function displayAttends($result)
{
   echo "<br>Attends : <br>";
              echo "<table border=\"1\">";
              echo "<tr><th>Ticket Number</th><th>Tournament Name</th></tr>";

             while ($row = OCI_Fetch_Array($result, OCI_BOTH)) {
                  echo "<tr><td>  " . $row["TICKETNUMBER"] . "  </td><td>  " . $row["TOURNAMENTNAME"] . "  </td></tr>";
            }
...
function handleDisplayAllAttendsRequest(){
    global $db_conn;
    $result = executePlainSQL("SELECT * FROM attends");
    displayAttends($result);
 
}

function displayTournaments($result){
echo "<br>Tournaments : <br>";
             echo "<table border=\"1\">";
             echo "<tr><th>TOURNAMENT NAME</th><th>TICKET PRICE</th><th>GRAND PRIZE</th><th>ORG NAME</th><th>ADDRESS</th><th>ARENA NAME</th><th>TOURNAMENT DATE</th><th>TOURNAMENT TIME</th></tr>";
             while ($row = OCI_Fetch_Array($result, OCI_BOTH)) {
                  echo "<tr><td>  " . $row["TOURNAMENTNAME"] . "  </td><td>  " . $row["TICKETPRICE"] . "  </td><td>  " . $row["GRANDPRIZE"] . "  </td><td>  " . $row["ORGNAME"] . "  </td><td>  " . $row["ADDRESS"] . "  </td><td>  " . $row["ARENANAME"] . "  </td><td>  " . $row["TOURNAMENTDATE"] . "  </td><td>  " . $row["TOURNAMENTTIME"] . "  </td></tr>";
            }
...
function  handleDisplayAllTournamentsRequest(){
    global $db_conn;
    $result = executePlainSQL("SELECT * FROM Tournament");
...
function handleGETRequest()
{
    if (connectToDB()) {
        ...
        }else if (array_key_exists('displayGamesTuples',$_GET)){
                 handleDisplayGamesRequest();
        }else if (array_key_exists('displayAllGamesTuples',$_GET)){
                 handleDisplayAllGamesRequest();
        }else if (array_key_exists('displayAllSpectatorsInfoTuples',$_GET)){
                 handleDisplayAllSpectatorsInfoRequest();
        }else if (array_key_exists('displayAllSpectatorsTicketsTuples',$_GET)){
                 handleDisplayAllSpectatorsTicketsRequest();
        }else if (array_key_exists('displayAllAttendsTuples',$_GET)){
                 handleDisplayAllAttendsRequest();
        }else if (array_key_exists('displayAllTournamentsTuples',$_GET)){
                 handleDisplayAllTournamentsRequest();
...
if (isset($_POST['reset']) || isset($_POST['updateSubmit']) || isset($_POST['insertSubmit'])) {
    ...
    handleGETRequest();
            ...
            handleGETRequest();
}else if (isset($_GET['displayGamesTupleRequest'])){
           handleGETRequest();
}else if (isset($_GET['displayAllGamesTupleRequest'])){
          handleGETRequest();
}else if (isset($_GET['displayAllSpectatorsInfoRequest'])){
          handleGETRequest();
}else if (isset($_GET['displayAllSpectatorsTicketsRequest'])){
          handleGETRequest();
}else if (isset($_GET['displayAllAttendsRequest'])){
          handleGETRequest();
}else if (isset($_GET['displayAllTournamentsRequest'])){
          handleGETRequest();

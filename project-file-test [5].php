...
<h2>Display the organizations for which their average tournament prize is the max over all tournaments</h2>
<form method="GET" action="project-file-test.php"> <!--refresh page when submitted-->
...
function connectToDB() {
    ...
    $db_conn = OCILogon("ora_ajiang03", "a90858895", "dbhost.students.cs.ubc.ca:1522/stu");
...
function handleGETRequest() {
    if (connectToDB()) {
        ...
        if (array_key_exists('displayTeamTuples',$_GET)){
            handleDisplayTeamRequest();
        }
        if (array_key_exists('displayOrgNameTuples',$_GET)){
